...
async function _consumeSSE(res, { onToken, onDone, onError, onInterrupted, onRejected, initialThreadId = '' }) {
  ...
  while (true) {
    ...
    for (const line of lines) {
      ...
      // ★ 换行还原：api.py 的 _stream_graph_run 用 \u2028（Unicode 行
      //   分隔符）转义了 token 内部真正的换行，避免多行内容破坏 SSE
      //   的单行 data: 帧格式（同时保持"读到一行就立刻派发"的实时性，
      //   不需要等到空行才能确认消息结束——见 v2 版协议说明）。
      //   这里对称地把 \u2028 还原回 \n，用于 markdown 渲染时正确换行。
      const data = line.slice(6).split('\u2028').join('\n')
      ...
      if (thisEvent === 'rejected') {
        ...
        stop(() => onRejected?.(payload, resolvedThreadId))
        return
      }
...
export function apiChat(question, thread_id = '') {
  return new Promise((resolve, reject) => {
    ...
    apiChatStream({
      ...
      onDone:  (tid)     => resolve({ answer: fullText, thread_id: tid || resolvedThreadId, interrupted: false, rejected: false, gateItems: [] }),
      ...
      // ★ 改动：不再 reject —— "本轮请求被 409 拒绝"跟 onInterrupted 一样，
      //   是后端给出的一种正常业务结果，不是网络/程序异常，不该走 .catch()。
      //   统一 resolve 一个 rejected:true 的结果，交给调用方（ChatPanel）
      //   在 .then() 里像处理 interrupted 一样处理它。
      onRejected: (payload, tid) => {
        ...
        resolve({
          ...
          interrupted: false,
          rejected:    true,
          message:     payload.message || '当前会话存在待人工处理的任务，请先前往「人工审核」面板处理',
          gateItems:   payload.pending_gate_items || [],
        })
      },
    })
...
// ── ★ Guardrail 改动：审计日志查询 + 规则开关 ─────────────
// → Next.js GET /api/guardrail/events?... → Python GET /guardrail/events?...
// params 可选：{ user_id, thread_id, stage, limit }，不传的字段不会拼进 query string
export const apiGetGuardrailEvents = (params = {}) => {
  ...
  return request(`/guardrail/events${suffix}`)
}

// → Next.js GET /api/guardrail/rules → Python GET /guardrail/rules
export const apiGetGuardrailRules = () => request('/guardrail/rules')

// → Next.js PUT /api/guardrail/rules/{category}?enabled=true|false
//   → Python PUT /guardrail/rules/{category}?enabled=true|false
export const apiSetGuardrailRule = (category, enabled) =>
  request(`/guardrail/rules/${encodeURIComponent(category)}?enabled=${enabled ? 'true' : 'false'}`, { method: 'PUT' })
