...
// ── Chat 非流式（兼容）─────────────────────────────────
// → Next.js POST /api/chat → Python POST /chat
export const apiChat = (question, thread_id = '') =>
  request('/chat', { method: 'POST', body: JSON.stringify({ question, thread_id }) })
...
// ── LangSmith Traces ────────────────────────────────────
export const apiListTraces = ({ limit = 20, filter = '', cursor = '' } = {}) => {
  const p = new URLSearchParams()
  if (limit)  p.set('limit',  limit)
  if (filter) p.set('filter', filter)
  if (cursor) p.set('cursor', cursor)
  return fetch(`/api/traces?${p}`).then(r => r.json())
}

export const apiGetTrace = (runId) =>
  fetch(`/api/traces/${runId}`).then(r => r.json())
