...
//   prefs     — 用户偏好（主题、界面配置等）
//   logs      — MemoryPanel 操作日志（持久化，最多保留 100 条）
//
// 文件位置：
...
// ── Logs CRUD ─────────────────────────────────────────
// 最多保留 LOG_MAX 条，写入时自动清理最旧的超出部分

const LOG_MAX = 100

export async function dbListLogs() {
  await initDb()
  const db = getDb()
  // 最新的排最前，和 MemoryPanel 原来的行为一致
  const res = await db.execute(
    `SELECT * FROM logs ORDER BY created_at DESC LIMIT ?`, [LOG_MAX]
  )
  return res.rows.map(rowToObj)
}

export async function dbAddLog(type, msg) {
  await initDb()
  const db = getDb()
  const id = newId()
  await db.execute(
    `INSERT INTO logs (id, type, msg) VALUES (?, ?, ?)`,
    [id, type, msg]
  )
  // 超出上限时删除最旧的
  await db.execute(
  ...
  const row = await db.execute(`SELECT * FROM logs WHERE id = ?`, [id])
  return rowToObj(row.rows[0])
}
