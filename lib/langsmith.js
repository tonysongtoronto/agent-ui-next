 ...
 * /runs/query 不支持 offset，必须用 lt(start_time, cursor) 翻页
...
export async function listRuns({ limit = 20, filter, cursor = null } = {}) {
  ...
  console.log(`[LangSmith] cursor=${cursor ?? 'start'} limit=${limit} got=${list.length}`)

  // 下一页游标 = 最后一条的 start_time
  const nextCursor = list.length > 0 ? list[list.length - 1].start_time : null

  return {
    runs:       list.map(normalizeRun),
    hasMore:    list.length === limit && nextCursor != null,
    nextCursor,
    project:    LS_PROJ,
  }
}
