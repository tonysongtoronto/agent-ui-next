...
/**
 * ★ 新增：Resume 流式代理——把 Python 新增的 SSE 恢复接口透传给浏览器
 *
 * Python api.py 的流式恢复接口是：
 *   POST /session/{user_id}/{thread_id}/resume/stream
 *   Body: { decisions: [...] }
 *
 * 跟 proxySSE（/chat/stream）几乎一样，唯一区别是这次 Python 那边也是
 * POST + JSON body，所以直接把 body 原样转发即可，不用像 proxySSE 那样
 * 把 POST 转成 GET query string。
 *
 * @param {string} thread_id
 * @param {string} userId
 * @param {string} body        - 原始请求体文本（{"decisions":[...]}），原样转发
 * @returns {Response}  text/event-stream
 */
export async function proxyResumeSSE(thread_id, userId, body) {
  const url = `${PYTHON_API}/session/${encodeURIComponent(userId)}/${thread_id}/resume/stream`
...
/**
 * ★ 新增：Abort 流式代理——把 Python 新增的 SSE 终止接口透传给浏览器
 *
 * Python api.py 的流式终止接口是：
 *   POST /session/{user_id}/{thread_id}/abort/stream
 *   （不需要 body，跟非流式的 /abort 一样）
 *
 * 跟 proxyResumeSSE 几乎一样，唯一区别是这里不用转发 body（abort 不需要
 * decisions），只是把 GET 换成 POST 空 body 打过去。
 *
 * @param {string} thread_id
 * @param {string} userId
 * @returns {Response}  text/event-stream
 */
export async function proxyAbortSSE(thread_id, userId) {
  const url = `${PYTHON_API}/session/${encodeURIComponent(userId)}/${thread_id}/abort/stream`
  ...
  let pythonRes
  try {
    pythonRes = await fetch(url, { method: 'POST' })
  } catch (err) {
    const msg = err.message.includes('ECONNREFUSED') || err.message.includes('fetch failed')
      ? `无法连接 Python 后端 (${PYTHON_API})。请确认 api.py 已启动：uvicorn api:app --port 8000`
      : err.message
